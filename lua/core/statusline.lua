...
local function git_info()
	...
	if diff.added > 0 then
		git_info =
			table.concat({ git_info, spaces[1], hl_groups["Green"], "+", diff.added, hl_groups["StatusLine"] }, "")
	end
	if diff.changed > 0 then
		git_info =
			table.concat({ git_info, spaces[1], hl_groups["Orange"], "~", diff.changed, hl_groups["StatusLine"] }, "")
	end
	if diff.removed > 0 then
		git_info =
			table.concat({ git_info, spaces[1], hl_groups["Red"], "-", diff.removed, hl_groups["StatusLine"] }, "")
	end
...
local function ruler()
	return get_mode_hl():gsub("#$", "Inv#") .. spaces[2] .. "%7(%l/%3L%):%2c %P" .. spaces[2] .. hl_groups["StatusLine"]
end
